...
Обязательное состояние панели хранится отдельно в `data_dir/panel-state.json`: сессии, настройки входа, nonce, политики, TOTP/passkeys, ограниченный аудит и recovery-журнал обновлений. SQLite содержит только историю наблюдаемости. Ею можно управлять на странице **Сервер → Настройки панели → История и хранение**: технические метрики обязательны, а события, проблемы подключений, общий и пользовательский трафик и расширенная диагностика имеют независимые переключатели и сроки хранения. Отключение категории останавливает новые записи, но не удаляет накопленное; очистка запускается отдельным подтверждённым действием.
...
Full-вариант также поддерживает PostgreSQL и MySQL/MariaDB. DSN хранится только в конфиге с правами `0600`; проверить его без запуска панели можно командой `telemt-panel store check`. Если сетевая база недоступна при старте, только история переключается во временную RAM; вход, настройки и обновления продолжают работать из локального state-файла. Возврат к постоянной базе выполняется перезапуском: автоматического слияния истории нет. Для резервной копии и переноса предусмотрены `telemt-panel store export/import`; history-часть импортируется только в пустую базу. Полные примеры — в [`docs/CONFIG.md`](docs/CONFIG.md).
