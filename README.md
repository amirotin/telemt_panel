...
Удаление: `uninstall` сохраняет конфиг и данные, `purge` удаляет указанные каталоги.
Telemt, внешние файлы и учётная запись ОС сохраняются. Для старого или уже
удалённого бинарника укажите `--binary` с актуальной панелью 1.x для безопасного
разбора путей. [Подробности и ограничения](docs/REMOVAL.md).
...
и сохраняет неприменяемые старые параметры в архиве состояния. Требуется остановленная
...
После успешного запуска 1.x можно явно выгрузить текущую схему командой
`config export --config OLD --out NEW`. Старый файл и состояние не меняются;
[переключение службы выполняется отдельно](docs/UPGRADING.md#экспорт-конфига-в-текущую-схему).
