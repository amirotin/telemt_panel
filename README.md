...
До релиза 1.0 продолжается функциональная доработка и стабилизация. В истории IP доступна локальная география (страна, ASN, опционально город); источники и обновления баз настраиваются в разделе «Сервер → Настройки панели → География IP». Подробнее — [настройка GeoIP](docs/CONFIG.md#география-ip-geoip).
