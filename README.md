...
До релиза 1.0 продолжается функциональная доработка и стабилизация. Отдельной волной после 1.0 — auth-extras: TOTP, passkeys/WebAuthn и `/api/geoip`.
...
|---|---|---|
| вариант | full: memory, SQLite, PostgreSQL, MySQL | lite: memory |
...
| хранилище | SQLite: обязательное состояние + настраиваемая история | memory: состояние в зеркале, история только в RAM |
...
SQLite хранит обязательное состояние панели транзакционно: сессии, настройки входа, nonce, параметры панели и recovery-журнал обновлений. История наблюдаемости управляется отдельно на странице **Сервер → Настройки панели → История и хранение**: технические метрики обязательны, а события, аудит, проблемы подключений, общий и пользовательский трафик и расширенная диагностика имеют независимые переключатели и сроки хранения. Отключение категории останавливает новые записи, но не удаляет накопленные; очистка запускается отдельным подтверждённым действием.

На procd/OpenWrt установщик оставляет `driver = "memory"`: история ограничена RAM-кольцом и пропадает после перезапуска, чтобы не создавать постоянную запись на флеш. Обязательное состояние при заданном `data_dir` продолжает зеркалироваться в `panel-state.json`. При ручном переходе с memory на SQLite существующее зеркало импортируется в пустую базу одной транзакцией и не удаляется автоматически.

Full-вариант также поддерживает PostgreSQL и MySQL/MariaDB. DSN хранится только в конфиге с правами `0600`; проверить его без запуска панели можно командой `telemt-panel store check`. Если настроенная сетевая база недоступна при старте, панель продолжит работу на временном RAM-хранилище и покажет предупреждение. Возврат к постоянной базе выполняется перезапуском: автоматического переключения и слияния истории нет. Для переноса между SQL-драйверами предусмотрены `telemt-panel store export/import`; импорт разрешён только в пустую базу. Полные примеры — в [`docs/CONFIG.md`](docs/CONFIG.md).
