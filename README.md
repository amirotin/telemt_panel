...
| `[telegram]` | `bot_script` | Путь к `bot.py` | `<data_dir>/bot/bot.py` (извлекается из бинарника) |
...
> **Требование:** на сервере должен быть установлен Python 3.8+. `bot.py` и `requirements.txt` извлекаются из бинарника автоматически в `<data_dir>/bot/` при каждом запуске панели. Зависимости Python устанавливаются автоматически через `pip` при первом старте бота — ничего дополнительно устанавливать вручную не нужно.
...
При каждом запуске панель автоматически извлекает `bot.py` и `requirements.txt` из бинарника в директорию `<data_dir>/bot/` (по умолчанию `/var/lib/telemt-panel/bot/`). Файл всегда актуален — обновляется вместе с панелью.
