...
> **Telemt Panel 1.0** готовится к выпуску в ветке `next`.
> Последний стабильный релиз старого поколения —
> [0.6.2](https://github.com/amirotin/telemt_panel/releases/tag/v0.6.2).
> При выпуске 1.0 основной веткой станет `main`, а код 0.6 перейдёт в `legacy`.
> Пока перенос не выполнен, `main` содержит старую панель; для проверки 1.0
> используйте `next` и тестовый стенд. [Порядок перехода веток](docs/RELEASING.md#ветки-и-версия-документации).
...
поэтому команда не зависит от переименования веток.
...
До публикации 1.0 — установщик из `next` и локально собранная панель
(на тестовом стенде):
