...
| `[telegram]` | `default_max_tcp_conns` | Лимит TCP-сессий по умолчанию для новых клиентов | `50` |
| `[telegram]` | `default_max_unique_ips` | Лимит уникальных IP (бот уведомит при превышении) | `5` |

## Telegram Bot

![Telegram Bot](docs/screenshots/telegram-bot.png)

Бот встроен в бинарник панели как горутина — Python не требуется.

### Возможности бота

- Регистрация пользователей через заявки (одобрение / отклонение администратором)
- Персональная MTProxy-ссылка с QR-кодом
- Статистика трафика и активных IP на пользователя
- Меню администратора: статистика всех клиентов, рассылка, блэклист, резервная копия БД
- Пересылка сообщений клиентов администраторам с поддержкой Reply
- Мониторинг доступности Telemt API с уведомлением в Telegram
- Уведомление администраторов о перезапуске панели
- Двуязычный интерфейс (EN/RU) — определяется по `language_code` пользователя

### Настройка

Откройте страницу **/telegram** в панели:

- **Bot Token** — вставьте токен, полученный от `@botfather`
- **Admin User IDs** — Telegram User ID администраторов (по одному на строку); узнать свой ID можно через `@userinfobot` или командой `/id` в боте
- **Max sessions per client** — лимит TCP-соединений при создании нового пользователя (по умолчанию: 50)
- **Max unique IPs per client** — при превышении бот уведомит администраторов (по умолчанию: 5)
- Нажмите **Save**, затем **Start**

Настройки сохраняются в раздел `[telegram]` файла `config.toml`. При следующем запуске панели бот стартует автоматически.

### Автоопределение домена прокси

Бот автоматически считывает домен прокси, порт и TLS-домен из конфига Telemt. Укажите путь к конфигу в `config.toml` через параметр `telemt.config_path`.

**Docker:** убедитесь, что `telemt.toml` примонтирован в контейнер панели:
...
**Bare metal:** если Telemt и панель на одной машине:
...
| Действие | Описание |
|----------|----------|
| **Start** | Запускает горутины бота, устанавливает `enabled = true` в config.toml |
| **Stop** | Останавливает бота, устанавливает `enabled = false` |
| **Save** | Сохраняет токен и ID администраторов; перезапускает бота если он был запущен |
| Статус-бейдж | Обновляется каждые 3 с; показывает «Running», «Stopped» или «Error» |
