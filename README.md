...
> **Требование:** на сервере должен быть установлен Python и зависимости бота. `bot.py` и `requirements.txt` извлекаются из бинарника автоматически в `<data_dir>/bot/` при каждом запуске панели — достаточно один раз выполнить:
> ```bash
> pip install -r /var/lib/telemt-panel/bot/requirements.txt
> ```
