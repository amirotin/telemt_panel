...
- **Тип:** массив строк (CIDR или одиночные IP), опционально.
- **По умолчанию:** пусто — заголовки `X-Forwarded-*` игнорируются.
- **Код:** парсинг в `config.Load`; использование в [`internal/server/client_ip.go`](../internal/server/client_ip.go).

Список обратных прокси, чьим заголовкам панель доверяет. Влияет на два механизма: rate-limit логина берёт клиентский IP из `X-Forwarded-For` (правый не-доверенный адрес) только если запрос пришёл от доверенного прокси — иначе подделка заголовка позволяла бы обходить лимит; флаг `Secure` у сессионной cookie и заголовок HSTS ставятся за TLS-терминирующим прокси по `X-Forwarded-Proto: https` — тоже только от доверенного адреса.

Пример для прокси на том же хосте: `trusted_proxies = ["127.0.0.1/32", "::1/128"]`.
