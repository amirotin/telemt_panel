...
export function TransportStatus() {
  ...
  const query = useQuery({ ...getPanelTlsOptions(), refetchInterval: 60_000 });
  const configQuery = useQuery({ ...getPanelTlsConfigOptions(), refetchInterval: 60_000 });
  const data = query.data;
  const behindHttps = data?.mode === "http" && window.location.protocol === "https:";
  const plain = data?.mode === "http" && !behindHttps;
  const state = data?.state;
  const stages: Record<string, string> = copy.stages;
  return (
    <section className="rounded-xl bg-surface p-4" data-testid="panel-transport">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-[15px] font-semibold text-text">{copy.title}</h3>
        {data && !query.isError && <StatePill state={plain || state === "warning" ? "warn" : state === "error" ? "error" : state === "waiting" ? "muted" : "ok"}>
          {behindHttps ? copy.proxy : copy.modes[data.mode]}
        </StatePill>}
      </div>
      {query.isPending ? <Skeleton className="mt-3 h-8" /> : query.isError ? (
        ...
        <div className="mt-2 space-y-2 text-meta text-text-muted">
          ...
          {state === "waiting" && <p>{copy.waiting}</p>}
          {state === "warning" && <p className="text-warning-text">{copy.warning}</p>}
          {state === "error" && <p className="text-error-text">{copy.failed}</p>}
          {data.error && <p className="break-words text-error-text">{data.stage ? `${stages[data.stage] ?? copy.title}: ` : ""}{data.error}</p>}
          {plain && <p className="text-warning-text">{copy.httpWarning}</p>}
          ...
          <p className="text-text-faint">{copy.configNote}</p>
          <Button type="button" variant="secondary" className="mt-1 w-full sm:w-auto" onClick={() => setFormOpen(true)}>
            {copy.configure}
          </Button>
        </div>
      )}
      {formOpen && <PanelAccessForm onClose={() => setFormOpen(false)} />}
    </section>
  );
}
