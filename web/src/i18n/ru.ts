  ...
  details: {
    ...
    pages: {
      ...
      web: {
        ...
        confirmFilterAll:
          "Будут закрыты ВСЕ живые WEB-сессии: фильтр не выбран. Клиентам придётся переподключиться.",
        confirmFilterTemplate:
          "Будут закрыты все живые WEB-сессии, подходящие под фильтр ({filter}). Отбор выполняет сам прокси, поэтому под него могут попасть и сессии, которых нет в загруженном списке.",
