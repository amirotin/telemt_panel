...
describe("formatLogClock", () => {
  it("renders a tabular HH:MM:SS clock", () => {
    expect(formatLogClock("2026-08-25T12:34:56Z")).toMatch(/^\d{2}:\d{2}:\d{2}$/);
  });

  it("falls back to the raw string for an unparseable ts", () => {
    expect(formatLogClock("not-a-date")).toBe("not-a-date");
  });
});

describe("formatAuditTimestamp", () => {
  it("renders day.month plus a clock", () => {
    expect(formatAuditTimestamp("2026-08-25T12:34:56Z")).toMatch(/^\d{2}\.\d{2},?\s+\d{2}:\d{2}:\d{2}$/);
  });
