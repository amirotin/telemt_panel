...
describe("personMeta", () => {
  it("shows connections · IPs · traffic for someone online", () => {
    const meta = personMeta({ user: user(3, 2), status: "active", quota }, s);
    expect(meta.tone).toBe("muted");
    expect(meta.text).toBe("3 соед. · 2 IP · 12 ГБ из 50 ГБ");
  });

  it("omits the limit for an unlimited quota", () => {
    expect(personMeta({ user: user(1), status: "active", quota: unlimited }, s).text).toBe(
      "1 соед. · 1 IP · 3.0 ГБ",
    );
  });

  it("falls back to an idle line with no connections", () => {
    const meta = personMeta({ user: user(0), status: "active", quota }, s);
    expect(meta.text).toBe("Не в сети · 12 ГБ из 50 ГБ");
  });

  it("lets a problem status win over the activity line", () => {
    expect(personMeta({ user: user(4), status: "quota_exhausted", quota }, s).tone).toBe("error");
    expect(personMeta({ user: user(4), status: "quota_exhausted", quota }, s).text).toContain(
      "Квота исчерпана",
    );
    expect(personMeta({ user: user(4), status: "expired", quota }, s).tone).toBe("warn");
    expect(personMeta({ user: user(4), status: "disabled", quota }, s).tone).toBe("faint");
    expect(personMeta({ user: user(4), status: "not_in_runtime", quota }, s).tone).toBe("warn");
  });
});

describe("personBadge", () => {
  it("shows the live connection count for an active, connected user", () => {
    expect(personBadge({ user: user(3), status: "active" }, s)).toEqual({ text: "3", tone: "accent" });
  });

  it("shows nothing for an idle but healthy user", () => {
    expect(personBadge({ user: user(0), status: "active" }, s)).toBeNull();
  });

  it("shows a word for each attention state", () => {
    expect(personBadge({ user: user(0), status: "quota_exhausted" }, s)?.tone).toBe("error");
    expect(personBadge({ user: user(0), status: "expired" }, s)?.tone).toBe("warn");
    expect(personBadge({ user: user(0), status: "disabled" }, s)?.tone).toBe("muted");
    expect(personBadge({ user: user(0), status: "not_in_runtime" }, s)?.tone).toBe("warn");
  });
});
