...
// Unit labels live in the dictionaries (people.bitrateUnits), not here.
export function formatBitsPerSecond(bps: number, s: Dict): string {
  const units = s.people.bitrateUnits;
  let value = bps;
  let unit = 0;
