...
// whether the underlying feature is on). `dcs.dcs` is typed as always an
// array, but a nil Go slice with no `omitempty` marshals as JSON `null`
// (confirmed against the live mock server), so this normalizes that to []
// rather than letting a null propagate into a `.map()` call downstream.
export function computeDc(
  dcs: { middle_proxy_enabled: boolean; reason?: string; dcs: DcStatus[] | null } | null,
): DcResult {
