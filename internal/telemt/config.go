...
func (c *Client) PatchConfig(ctx context.Context, patch map[string]any, revision string, reload ReloadQuery) (PatchConfigResult, string, error) {
	path := "/v1/config"
	if reload.Mode != "" {
		q := url.Values{}
	...
	return mutateRevision[PatchConfigResult](ctx, c, http.MethodPatch, path, patch, revision)
}
