...
func validatePortableMetric(name string, point MetricPoint) error {
	if name == "" {
		return fmt.Errorf("metric name cannot be empty")
	}
	switch point.Tier {
	...
	// Exports predating aggregate metadata intentionally leave these fields
	// absent. Do not invent observation bounds for legitimate schema-11 rows.
	if point.Min == nil {
		return nil
	}
	width := int64(3600)
